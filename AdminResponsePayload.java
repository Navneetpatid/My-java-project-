...
pipeline {
  ...
  parameters {
    ...
    // If you paste a full JSON body here, field params below are ignored.
    text(name: 'ADD_JSON', defaultValue: '', description: 'Optional: full JSON body for ADD API. Leave empty to use field parameters.')
    // Field params (used only when ADD_JSON is empty). Match your Insomnia JSON:
  ...
  environment {
    ...
    TOKEN_PATH = '/oauth2/token'            // change if your token path is different
    ADD_PATH   = '/cer/add/shp/data'        // from your Insomnia curl
  }
  ...
  stages {
        ...
        withCredentials([usernamePassword(credentialsId: 'svc_api_client', usernameVariable: 'CLIENT_ID', passwordVariable: 'CLIENT_SECRET')]) {
          script {
            ...
            if (params.AUTH_MODE == 'oauth2_form') {
              ...
              // OAuth2 client_credentials (JSON body)
              tokenResp = sh(
                script: """#!/usr/bin/env bash
                  set -e
                  curl -sS --fail-with-body -X POST "${AUTH_URL}" \\
      ...
      steps {
        script {
          if (params.ADD_JSON?.trim()) {
            ...
            // Build JSON from fields (matching your Insomnia body)
            def payload = [
            ...
            ].findAll { k,v -> v != null && v.toString().trim() } // drop null/empty

            // Minimal required fields sanity (edit if your API requires more)
    ...
    stage('Call ADD API') {
      steps {
        script {
          // Build optional Cookie header if requested
          def cookieHeader = ''
          ...
          // Try up to 3 times (handles transient 5xx)
          int attempts = 3
          int delaySec = 5
          String httpCode = ''
          for (int i=1; i<=attempts; i++) {
            try {
              ...
              // Exit loop if success HTTP (200/201/202)
          ...
          // Try to show created ID if present
          try {
